...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
         ...
         * @var string
         */
        public $menu = 'hohb-install-plugins';
         ...
         * does the hohb_init action hook, and hooks in the interactions to init.
         *
         * @since 1.0.0
         *
        ...
        public function __construct() {
            ...
            do_action_ref_array( 'hohb_init', array( $this ) );
        ...
        public function init() {
            ...
            // Load class strings.
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'hohb' ),
                'menu_title'                     => __( 'Install Plugins', 'hohb' ),
                'installing'                     => __( 'Installing Plugin: %s', 'hohb' ),
                'oops'                           => __( 'Something went wrong.', 'hohb' ),
                'notice_can_install_required'    => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'hohb' ),
                'notice_can_install_recommended' => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'hohb' ),
                'notice_cannot_install'          => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'hohb' ),
                'notice_can_activate_required'   => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'hohb' ),
                'notice_can_activate_recommended'=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'hohb' ),
                'notice_cannot_activate'         => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'hohb' ),
                'notice_ask_to_update'           => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'hohb' ),
                'notice_cannot_update'           => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'hohb' ),
                'install_link'                   => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'hohb' ),
                'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'hohb' ),
                'return'                         => __( 'Return to Required Plugins Installer', 'hohb' ),
                'dashboard'                      => __( 'Return to the dashboard', 'hohb' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'hohb' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'hohb' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'hohb' ),
                'dismiss'                        => __( 'Dismiss this notice', 'hohb' ),
            );

            do_action( 'hohb_register' );
         ...
         *
         * We get the links in the admin notices to point to the hohb page, rather
         ...
         * @return null Returns early if not the hohb page.
         */
        ...
        public function thickbox() {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'hohb_dismissed_notice_' . $this->id, true ) ) {
                add_thickbox();
            }
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new hohb_List_Table;

            // Return early if processing a plugin installation action.
            if ( isset( $_POST['action'] ) && 'hohb-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            }
            ...
            <div class="hohb wrap">
                ...
                <?php if ( version_compare( $this->wp_version, '3.8', '<' ) ) {
                    screen_icon( apply_filters( 'hohb_default_screen_icon', 'themes' ) );
                } ?>
                ...
                <form id="hohb-plugins" action="" method="post">
                    <input type="hidden" name="hohb-page" value="<?php echo $this->menu; ?>" />
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['hohb-install'] ) && 'install-plugin' == $_GET['hohb-install'] ) ) {
                check_admin_referer( 'hohb-install' );
                ...
                $fields = array( 'hohb-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'hohb' ) . '">' . __( 'Return to the Dashboard', 'hohb' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['hohb-activate'] ) && 'activate-plugin' == $_GET['hohb-activate'] ) ) {
                check_admin_referer( 'hohb-activate', 'hohb-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'hohb' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'hohb' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'hohb' ) . '</a>'  : '';

                // Define all of the action links.
                $action_links = apply_filters(
                    'hohb_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . add_query_arg( 'hohb-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'hohb', 'hohb', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'hohb', 'hohb', $rendered, $nag_class );
                }
            }

            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'hohb' );
            }
        ...
        public function dismiss() {
            ...
            if ( isset( $_GET['hohb-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'hohb_dismissed_notice_' . $this->id, 1 );
            }
        ...
        public function actions( $install_actions ) {
            ...
            // Remove action links on the hohb install page.
            if ( $this->is_hohb_page() ) {
                return false;
            }
        ...
        /**
         * Determine if we're on the hohb Install page.
         *
         * @since 2.1.0
         *
         * @return boolean True when on the hohb page, false otherwise.
         */
        protected function is_hohb_page() {
        ...
        public function update_dismiss() {
            ...
            delete_user_meta( get_current_user_id(), 'hohb_dismissed_notice_' . $this->id );
    ...
    $hohb = TGM_Plugin_Activation::get_instance();

}

if ( ! function_exists( 'hohb' ) ) {
    /**
     ...
     */
    function hohb( $plugins, $config = array() ) {
...
if ( ! class_exists( 'hohb_List_Table' ) ) {
    /**
     ...
     */
    class hohb_List_Table extends WP_List_Table {

        /**
         * References parent constructor and sets defaults for class.
         *
         * The constructor also grabs a copy of $instance from the hohb class
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'hohb' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'hohb' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'hohb' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'hohb' ) : __( 'Recommended', 'hohb' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'hohb' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'hohb' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'hohb' ) . ' %2$s">' . __( 'Install', 'hohb' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'hohb' ) . ' %2$s">' . __( 'Activate', 'hohb' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
                                'plugin_name'          => $item['sanitized_plugin'],
                                'plugin_source'        => $item['url'],
                                'hohb-activate'       => 'activate-plugin',
                                'hohb-activate-nonce' => wp_create_nonce( 'hohb-activate' ),
                            ),
                            admin_url( 'themes.php' )
                        ),
                        $item['sanitized_plugin']
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'hohb' ), admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'hohb' ),
                'source' => __( 'Source', 'hohb' ),
                'type'   => __( 'Type', 'hohb' ),
                'status' => __( 'Status', 'hohb' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'hohb-bulk-install'  => __( 'Install', 'hohb' ),
                'hohb-bulk-activate' => __( 'Activate', 'hohb' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'hohb-bulk-install' === $this->current_action() ) {
                ...
                if ( empty( $plugin_installs ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be installed at this time.', 'hohb' ) . '</p></div>';
                    return false;
                }
                ...
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'hohb-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'hohb-empty';
                    $i++;
                }
            ...
            if ( 'hohb-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
                ...
                if ( empty( $plugins_to_activate ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be activated at this time.', 'hohb' ) . '</p></div>';
                    return false;
                }
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'hohb' ), $imploded );
                }
 ...
 * @since 2.2.0
 */
add_action( 'admin_init', 'hohb_load_bulk_installer' );
function hohb_load_bulk_installer() {
    ...
    if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && TGM_Plugin_Activation::$instance->menu === $_GET['page'] ) ) {
        ...
        if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
            ...
            class TGM_Bulk_Installer extends WP_Upgrader {
                 ...
                 * set to true for the hohb class.
                 *
                 * @since 2.2.0
                 *
                ...
                public function install_strings() {
                    ...
                    $this->strings['no_package']          = __( 'Install package not available.', 'hohb' );
                    $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'hohb' );
                    $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'hohb' );
                    $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'hohb' );
                    $this->strings['process_failed']      = __( 'Plugin install failed.', 'hohb' );
                    $this->strings['process_success']     = __( 'Plugin installed successfully.', 'hohb' );
                ...
                public function activate_strings() {
                    ...
                    $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'hohb' );
                    $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'hohb' );
        ...
        if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
            ...
            class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                ...
                public function add_strings() {
                    ...
                    if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                        $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'hohb' );
                        $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'hohb' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'hohb' ) . '</span><span class="hidden">' . __( 'Hide Details', 'hohb' ) . '</span>.</a>';
                        $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'hohb' );
                        $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'hohb' );
                    }
                    // Default installation strings.
                    else {
                        $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'hohb' );
                        $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'hohb' );
                        $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'hohb' );
                        $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'hohb' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'hohb' ) . '</span><span class="hidden">' . __( 'Hide Details', 'hohb' ) . '</span>.</a>';
                        $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'hohb' );
                        $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'hohb' );
                    }
                ...
                public function bulk_footer() {
                    ...
                    if ( empty( $complete ) ) {
                        echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'hohb' ) . '">' . __( 'Return to the Dashboard', 'hohb' ) . '</a>' ) . '</p>';
                        echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                    }
